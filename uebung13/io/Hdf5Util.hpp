...
static void writeBaseStructure(std::shared_ptr<HighFive::File> hdf5_file)
{
    ...
    // Create empty reference frame (identity matrix)
    std::vector<float> frame(16);
    for (int i = 1; i < 15; i++)
    {
        frame[i] = 0;
    }
...
static HighFive::Group getGroup(std::shared_ptr<HighFive::File> hdf5_file,
                                                 ...
                                                 const HighFive::DataSetCreateProps& properties)
{
    ...
    if (g.exist(datasetName))
    {
        ...
        if (dataset->getDataType() != HighFive::AtomicType<T>())
        {
            ...
            H5Ldelete(g.getId(), datasetName.data(), H5P_DEFAULT);
            dataset = std::make_unique<HighFive::DataSet>(g.createDataSet<T>(datasetName, dataSpace, properties));
        }
        else if (dims_old[0] != dims_new[0] || dims_old[1] != dims_new[1])
        {
            ...
            catch (HighFive::DataSetException& ex)
            {
                ...
                dataset
                        = std::make_unique<HighFive::DataSet>(g.createDataSet<T>(datasetName, dataSpace, properties));
            }
        }
    }
    else
