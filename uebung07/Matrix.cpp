...
Matrix Matrix::operator+=(const Matrix& other) { return *this + other; }
...
void Matrix::toPostionAngle(float pose[6])
{
    if (pose != 0)
    {
        ...
        if (C == 0.0)
        {
            throw DivisionByZeroException("Matrix::toPostionAngle - Division by zero");
        }
        if (fabs(C) > 0.005)
        {                         // Gimball lock?
...
void Matrix::operator*=(const float scale) { *this = *this * scale; }

/**
 * @brief 	Matrix-Matrix multiplication with self assigment.
 */

void Matrix::operator*=(const Matrix& other) { *this = *this * other; }

/**
 * @brief	Matrix-Matrix multiplication (array based). See \ref{operator*}.
 */

void Matrix::operator*=(const float* other) { *this = *this * other; }
...
Matrix Matrix::inv(bool& success)
{
    ...
    if (mdet == 0.0)
    {
        throw DivisionByZeroException("Matrix: Division by zero");
    }
...
{
    float det;
    det = (double) (M[0] * (M[4] * M[8] - M[7] * M[5]) - M[1] * (M[3] * M[8] - M[6] * M[5])
                    + M[2] * (M[3] * M[7] - M[6] * M[4]));
    return (det);
}
