...
void Read3DS::getMesh(TexturedMesh::Ptr texMesh) 
{
...
//    TextureFactory::setBasePath(basePath); // cannot use texture factory base path because of hdf5
    ...
    for(size_t i = 0; i < matReader.materials().size(); i++)
    {
        ...
        if(m.texMaps.size() > 0)
        {
            std::string filename = m.texMaps[0].filename.substr(0, m.texMaps[0].filename.find_last_of('.'));
            mat->m_texture = TextureFactory::instance().getTexture(filename);
        }
        else
        {
            mat->m_texture = 0;
    ...
    for(size_t i = 0; i < normalMesh.meshes().size(); i++)
    {
        ...
        for(size_t j = 0; j < mesh.faceMaterials.size(); j++)
        {
            ...
            MaterialFaceList::Ptr matList(new MaterialFaceList);
...
void Read3DS::readSimpleMesh()
{
    ...
    // Alloc memory for buffers
    m_vertexBuffer  = FloatArray(new float [3 * m_numVertices]);
    m_normalBuffer  = FloatArray(new float [3 * m_numVertices]);
    m_indexBuffer   = IntArray(new int [3 * m_numFaces]);
...
auto Read3DS::getMesh() -> TriangleMesh::Ptr
{
	TexturedMesh::Ptr mesh(new TexturedMesh);
	getMesh(mesh);
