...
LevelParser::LevelParser(const std::string& file, SpaceCraft*& spaceCraft, Skybox*& sky, AsteroidField*& af)
{
    ...
    BOOST_FOREACH(ptree::value_type& v, tree.get_child("level"))
    {
        if (v.first == "skybox")
        {
            ...
            sky = new Skybox(v.second.get<int>("<xmlattr>.width", 1024), names);
        }
        else if (v.first == "asteroids")
        {
            af = new AsteroidField(v.second.get<int>("count", 0), 
        ...
        {
            spaceCraft = new SpaceCraft(path + v.second.get<std::string>("<xmlattr>.filename", "error"),
